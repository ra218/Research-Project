...
public class Main {
	...
	public static List getImpactedList(int thisNodeIndex,List nodeList) {
...
//		List impactedNodeList=(List) ((Node)nodeList.get(impactedNodeIndex)).outputNodes.get(impactedComponent);
		...
		List impactedNodeList=mapToList(((Node)nodeList.get(thisNodeIndex)).outputNodes);
...
//		timeFramesNeeded
		...
		//crating list of outputfl for all nodes.
		...
		if(((List)((Node)nodeList.get(i)).outputFlList.get(pair.getKey())).size()<3 ) {
			
			for(int gk=0;gk<timeFramesNeeded;gk++) {
				
				float bb=0;
				
				((List)((Node)nodeList.get(i)).outputFlList.get(pair.getKey())).add(bb);
			...
			for(int i=0;i<nodeList.size();i++) {
			    ...
			    while (it.hasNext()) {
			        ...
			        if(  ((Node)nodeListBeforeDisruption.get(i)).fullfilmentRate.get(pair.getKey())!= ((Node)nodeList.get(i)).fullfilmentRate.get(pair.getKey())   ) {
			        	...
			        	for(int k=2;k<(timeFramesNeeded+2);k++) {
			        		...
			        		if(newVal<previousVal) {
			        			...
			        			//System.out.println("Size of of list  = "+((List)((Node)nodeList.get(i)).outputFlList.get(pair.getKey())).size() );
...
//			        			}
			        			...
			        			float alternateRate=0;
			        			Double blessMe=getRandom();
				    // check if alternate services are available in this time frame (30%) probability		
			        			if(blessMe<=0.3) {
			        				
			        				alternateRate=((float)getRandom())*10;
					        		
					        		//identify impacted nodes for this node
			        				
			        List tempImpactedNodeList=getImpactedList(i,nodeList);
			        
			        for(int burger=0;burger<nodeList.size();burger++) {
			        	
			        	if(tempImpactedNodeList.contains(burger)) {
			        		float tValue=(float) ((List)((Node)nodeList.get(burger)).outputFlList.get(pair.getKey())).get(k);
					        ((List)((Node)nodeList.get(burger)).outputFlList.get(pair.getKey())).set(k,(tValue+alternateRate));
			        	}
			        	
			        	
			        }
			        				
			        				// add alternate Rate to kth index of outputfl for pair.getkey()
			        		...
			        		float tempVal=(float)((List)((Node)nodeList.get(i)).outputFlList.get(pair.getKey())).get(k);
					        ((List)((Node)nodeList.get(i)).outputFlList.get(pair.getKey())).set(k,(newVal+tempVal+alternateRate));
